...
interface CoroplastSignsLaminated {
  code: string;
...
export default function CoroplastSignsLaminated() {
  const [product, setProduct] = useState<CoroplastSignsLaminated[]>([]);
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/coroplast_signs_laminated')
  ...
  return (
    <>
       <OneSelect
        displayName='Coroplast Signs Laminated' 
        displayCode='(COR)'
        image={image}
        name={selectedName}
        size={selectedName}
